...
public class FiltradorEsquinaInferior extends Filtrador implements Runnable {

	public FiltradorEsquinaInferior(int[][] matrizBase, int id, int salto, Contenedor miContendor, int mayorOMenor) {
		super(matrizBase, id, salto, miContendor, mayorOMenor);
	}

	public void filtarIterativoEsquinaInferior() {
		for (int id = 0; id < this.matrizBase.length; id++) {
			for (int i = 0; i < this.matrizBase[id].length; i++) {
				int valor;
				if (this.validarFiltroEsquinaInferior(id, i)) {
	...
	public void run() {
		for (this.id = 0; this.id < this.matrizBase.length; this.id++) {
			for (int i = 0; i < this.matrizBase[this.id].length; i++) {
				int valor;
				if (this.validarFiltroEsquinaInferior(this.id, i)) {
					if (this.mayorOMenor == 1) {
